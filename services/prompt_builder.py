...
def build_project_system_prompt(base_prompt: str, project_title: str, ctx: dict) -> str:
    ...
    if project_title:
        parts[0] += (
            ...
            "Используй только сведения из блоков ниже (ПАМЯТЬ ПРОЕКТА, БРИФ ПРОЕКТА, "
            "ИСТОРИЯ ЧАТА, НОВЫЕ СООБЩЕНИЯ) — это единственные разрешённые источники. "
            "Никогда не используй и не упоминай сведения о других клиентских проектах "
            "агентства. Если нужной информации нет в этих источниках — прямо скажи "
            "«В памяти проекта нет этой информации», не придумывай и не предполагай."
        )
